...
class GmailApi : Api {
    ...
    override fun getMessages(uid: String, dialogId: String): List<Message> {
        ...
        return response.messageList.stream().map { d -> Message(d.sender, d.message, getTime(d.date), d.sender == "me", d.date, d.attachment.type, d.attachment.url) }.filter { it.attachementType.length >0 || it.text.length > 0 }.collect(Collectors.toList())
